...
@Injectable()
export class MasterQueriesDatabaseService {
    constructor(
        // @Inject('MONGO_MASTER') private _db: Db,
    ...
    async findOne(collectionName: string, query?: Record<string, any>, project?: Record<string, any>, sort?: Record<string, any>) {
        return Promise.resolve().then(() => {
            ...
            let find = this._databaseService.getConnetionTenant('RepoticsMaster').collection(collectionName).find(query).project(project);
    ...
    async find(collectionName: string, query?: Record<string, any>, project?: Record<string, any>, sort?: Record<string, any>, limit?: number, skip?: number) {
        return Promise.resolve().then(() => {
            ...
            let find = this._databaseService.getConnetionTenant('RepoticsMaster').collection(collectionName).find(query).project(project).skip(skip);

            if (sort) {
                find = find.collation({ locale: 'en', strength: 2 }).sort(sort);
            }
            if (limit) {
    ...
    async insert(collectionName: string, data?: Record<string, any>,id = "",includeTimestamps: boolean = true) {
        return Promise.resolve().then(() => {
            if (id) {
                data['_id'] = id;
            }else{
            ...
            //insert to insertOne
            return this._databaseService.getConnetionTenant('RepoticsMaster').collection(collectionName).insertOne(data);
        }).then((data) => {
            return Promise.resolve(data);
        }).catch((error) => {
    ...
    async insertWithSession(collectionName: string, data?: Record<string, any>, session?: ClientSession,id = "",includeTimestamps: boolean = true) {
        return Promise.resolve().then(() => {
            if (id) {
                data['_id'] = id;
            }else{
            ...
            //insert to insertOne
            return this._databaseService.getConnetionTenant('RepoticsMaster').collection(collectionName).insertOne(data, { session });
        }).then((data) => {
            return Promise.resolve(data);
        }).catch((err) => {
            throw new HttpException({
                status: HttpStatus.INTERNAL_SERVER_ERROR,
                error: "Something went wrong on server", details: err,
            }, HttpStatus.INTERNAL_SERVER_ERROR);
        });
    };
    ...
    async insertMany(collectionName: string, data?: Record<string, any>[], cid = 1){
        return Promise.resolve().then(() => {
            ...
            return this._databaseService.getConnetionTenant('RepoticsMaster').collection(collectionName).insertMany(data);
        }).then((data) => {
            return Promise.resolve(data);
        }).catch(() => {
            throw new HttpException({
    ...
    async update(collectionName: string, condition?: Record<string, any>, update?: Record<string, any>) {
        return Promise.resolve().then(() => {
            ...
            return this._databaseService.getConnetionTenant('RepoticsMaster').collection(collectionName).updateMany(condition, update);
        }).then((data) => {
            return Promise.resolve(data);
        }).catch(() => {
            throw new HttpException({
    ...
    async delete(collectionName: string, condition?: Record<string, any>, multi: number = 0) {
        return Promise.resolve().then(() => {
            if (!condition) {
            ...
            return this._databaseService.getConnetionTenant('RepoticsMaster').collection(collectionName)[action](condition);
        }).then((data) => {
            return Promise.resolve(data);
        }).catch(() => {
            throw new HttpException({
    ...
    async count(collectionName: string, condition?: Record<string, any>):Promise<number>{
        return Promise.resolve().then(() => {
            ...
            return this._databaseService.getConnetionTenant('RepoticsMaster').collection(collectionName).countDocuments(condition);
        }).then((data) => {
            return Promise.resolve(data);
        }).catch(() => {
            throw new HttpException({
    ...
    async aggregation(collectionName: string, query?: Record<string, any>[]) {
        return Promise.resolve().then(() => {
            ...
            return this._databaseService.getConnetionTenant('RepoticsMaster').collection(collectionName).aggregate(query).toArray();
