...
export function PaginationPagingPipe(defaultPerPage: number = PAGINATION_DEFAULT_LIMIT): Type<PipeTransform> {
    ...
    class MixinPaginationPagingPipe implements PipeTransform {
        ...
        async transform(value: Record<string, any>): Promise<Record<string, any>> {
            const page: number = value?.page  ? Number.parseInt(value?.page) : PAGINATION_DEFAULT_PAGE;
